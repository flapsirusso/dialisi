/*
  # Create complete database schema for shift management system

  1. New Tables
    - `teams` - Team definitions with locations and allowed shift codes
    - `staff` - Staff members with roles, contracts, and team assignments
    - `scheduled_shifts` - Individual shift assignments
    - `absences` - Staff absence records
  ...
  3. Data
    - Populate teams with initial data
    - Populate staff with initial data
    - Populate shift definitions with complete shift codes and styling
*/
...
CREATE POLICY "Authenticated users can read scheduled shifts"
  ON scheduled_shifts
  FOR SELECT
  TO authenticated
  USING (true);
...
-- Insert additional shift definitions that may be missing
